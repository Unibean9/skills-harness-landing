...
2. **Compute the slug**:
   ```bash
   node .cursor/skills/fk/scripts/critique-storage.mjs slug "<resolved-path-or-url>"
   ```
...
CLI scan:
```bash
node .cursor/skills/fk/scripts/detect.mjs --json [target]
```
...
4. If mutation is available, start `node .cursor/skills/fk/scripts/live-server.mjs --background`, present the browser if supported, label `[Human]`, scroll top, inject `http://localhost:PORT/detect.js`, wait 2-3 seconds, read `fk` console messages, then stop the live server.
...
- **Suggested command**: Which command could address this (from: /fk responsive, /fk motion, /fk check, /fk amplify, /fk copy, /fk color, /fk review, /fk joy, /fk trim, /fk spec, /fk prod, /fk space, /fk welcome, /fk perf, /fk wow, /fk finish, /fk calm, /fk plan, /fk type)

#### Persona Red Flags
> *Consult the [Personas reference](#persona-based-design-testing) below.*

Auto-select 2-3 personas most relevant to this interface type (use the selection table in the reference). If `.cursorrules` contains a `## Design Context` section from `fk init`, also generate 1-2 project-specific personas from the audience/brand info.
...
Once the report above is finalized, write it to `.fk-skills/critique/` so the user can refer back, and so `/fk polish` can pick up the priority issues without a copy-paste.
...
2. **Pass the structured metadata** through `FK_SKILLS_CRITIQUE_META` (JSON), then run the write command:
   ...
   FK_SKILLS_CRITIQUE_META='{"target":"<user phrasing>","total_score":<n>,"p0_count":<n>,"p1_count":<n>}' \
     node .cursor/skills/fk/scripts/critique-storage.mjs write <slug> <body-file>
   ```
...
4. **Read the trend** for context:
   ```bash
   node .cursor/skills/fk/scripts/critique-storage.mjs trend <slug> 5
   ```
...
- Only recommend commands from: /fk responsive, /fk motion, /fk check, /fk amplify, /fk copy, /fk color, /fk review, /fk joy, /fk trim, /fk spec, /fk prod, /fk space, /fk welcome, /fk perf, /fk wow, /fk finish, /fk calm, /fk plan, /fk type
...
- End with `/fk polish` as the final step if any fixes were recommended

After presenting the summary, tell the user:

> You can ask me to run these one at a time, all at once, or in any order you prefer.
>
> Re-run `/fk critique` after fixes to see your score improve.
...
If `.cursorrules` contains a `## Design Context` section (generated by `fk init`), derive 1–2 additional personas from the audience and brand information:
